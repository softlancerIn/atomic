...
<body>
    <div class="content-wrapper">
        <div class="container-xxl flex-grow-1 container-p-y">
            <div class="row">
                <div class="col-6">
                    <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Dashboard / </span>Crops /Edit Crop Disease</h4>
                </div>
                                    ...
                                    <form class="row g-3" action="{{route('crop_disease_create')}}" method="post" enctype="multipart/form-data">
                                        ...
                                        <div class="col-md-6">
                                            <select id="crops" class="form-select form-control" name="crop_id">
                                                ...
                                                <option value="{{$item->id}}" {{($data['crop_data']->crop_id == $item->id) ? 'selected':''}}>{{$item->name}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-6">
                                        ...
                                        <div class="col-md-6">
                                            <input type="text" class="form-control" name="name" placeholder="Crop Disease Name" aria-label="Owner Name" name="crop_name" value="{{$data['crop_data']->name}}">
                                        </div>
                                        <div class="col-md-6">
                                            <input type="file" class="form-control" aria-label="Mobile Number" name="image">
                                        </div>
                                        @php
                                            $slider_data = $data['crop_data']->slider_data;
                                            
                                        @endphp
                                        @foreach (json_decode($slider_data,true) as $key=>$item)  
                                            @if($key == '0')
                                                <div class="row g-3 add_moreRow">
                                            @else
                                                <div class="row g-3 remove_row{{$key}} mt-0">
                                            @endif
                                                <div class="col-md-6">
                                                    <label for="formGroupExampleInput" class="form-label">Crop Slider name (optional)</label>
                                                    <input type="text" class="form-control" aria-label="name" name="slider_name[]"value="{{$key}}">
                                                </div>
                                                <div class="col-md-4">
                                                    <label for="formGroupExampleInput" class="form-label">Slider Image (Optional)</label>
                                                    <input type="file" class="form-control" aria-label="Mobile Number" name="slider_image[]" value="{{$item}}">
                                                </div>
                                                <div class="col-md-2 text-end">
                                                    ...
                                                    @if($key == '0')
                                                    @else
                                                        {{-- <button type="button" class="btn btn-danger removeBtn${$i}" onclick="remove({{$key}})">Remove</button> --}}
                                                    @endif
                                                </div>
                                            </div>
                                        @endforeach
