...
<div class="content-wrapper">
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="row">
            <div class="col-6">
                <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Dashboard / </span>Sub Category /Edit Subcategory</h4>
            </div>
                        ...
                        <div class="col-12 col-md-8 col-lg-12">
                            <div class="bg-white">
                                <form class="row g-3" method="post" action="{{route('saveSubCategory')}}" enctype="multipart/form-data">
                                    ...
                                    <div class="col-md-6">
                                        <input type="text" class="form-control" name="name" placeholder="Enter Subcategory Name" value="{{$data['edit_data']->name}}" aria-label="Mobile Number" required>
                                    </div>
                                    <div class="col-md-6">
                                        <select name="cat_id" id="" class="form-select form-control">
                                            <option value="" selected disabled>Select Category</option>
                                            @foreach($data['category'] as $item)
                                            <option value="{{$item->id}}" {{$item->id == $data['edit_data']->cat_id ? 'selected' : ''}}>{{$item->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-6">
                                    ...
                                    <div class="col-md-6">
                                        <textarea class="form-control" name="description" placeholder="Description">{{$data['edit_data']->description}}</textarea>
                                    </div>
                                    <div class="col-12">
